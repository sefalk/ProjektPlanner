...
>
> **WP6** ist implementiert (Suite grün: 440 Tests, Coverage 91.60 %): zielorientiertes Rebalancing + Empfehlungen + WP1-Rebalancing-Teil (V7, V8, BUG-8/9/10). `suggest_rebalancing` verteilt jetzt das Restbudget `R = Gesamt − abgerechnet(gesperrt) − gebucht(offen) − Overrides` über die offenen Meilensteine per gemeinsamer `distribute_budget`/`_person_available_hours` (Netto-Kapazität statt roh → BUG-8), reizt R maximal aus ohne Überschreitung (BUG-9), berücksichtigt Drift/gebuchte Stunden (BUG-10) und lässt Override-Zeilen fix; `MilestoneSuggestion.suggested_total_hours` neu für die „Rebalanciert"-Spalte. `apply_rebalancing` rundet nicht mehr (§9.4). Neu: `compute_recommendations` (Umkehrung von `_membership_overbooking_warnings`) + Endpoint `GET /projects/{id}/milestones/recommendations` (frei h/Woche, €-Headroom, empfohlene Zusatzstunden). Tests: erweiterte `test_services_rebalancing.py` + Router-Tests. **Offen:** WP7–WP8 (Frontend + Anwenderdoku).
