...
> **Umsetzungsstand (02.07.2026):** WP1 (milestones-Teil), WP2 und **WP3** sind implementiert; die volle pytest-Suite läuft im Dev-Env grün (415 Tests, Coverage 90.96 %, `--cov-fail-under=90`). Die reine Verteilungslogik `distribute_budget` wurde gegen die harten Invarianten (Budget/Kapazität) und das Prioritätsverhalten verifiziert (inkl. Fuzz). WP3 ergänzt: Nullmeilenstein-Guard `NoActiveMembership` in `initialize_milestones` (§8.1, Router → HTTP 422); referenzielle Aktionen (V11) `remove_member_budgets`/`prune_member_budgets_to_range`/`apply_project_range_change` (in `delete_membership`/`update_membership`/`update_project` verdrahtet, gesperrte Monate geschützt, V10-Cache via `_resync_milestone_totals`); 0-h-/„außerhalb-Projektzeitraum"-Warnungen im `MilestoneDetailOut.warnings`. Tests: `test_services_milestones_referential.py`, ergänzte Router-Tests.
>
> **WP4** ist ebenfalls implementiert (Suite grün: 423 Tests, Coverage 91.51 %): nicht-destruktiver `resync_milestones`/`_align_open_milestones` (V5) — fehlende Budgetzeilen korrekt skaliert ergänzen (BUG-4), geänderte Wochenstunden neu rechnen (BUG-5), verwaiste/außerhalb-des-Zeitraums Zeilen entfernen (BUG-7), `is_manual_override`-Zeilen und gesperrte Monate unangetastet; Overrides + abgerechnete (gesperrte) Monate reduzieren R zuerst. Endpoint `POST /projects/{id}/milestones/resync` liefert `{added, removed, recomputed, changed_milestone_ids}`. Der fehlerhafte Repair-Pfad in `initialize_milestones` (ungeskaliert, BUG-4) ist durch Delegation an `_align_open_milestones` ersetzt. Tests: `test_services_milestones_resync.py` + Router-Test.
>
> **WP5** ist implementiert (Suite grün: 432 Tests, Coverage 91.47 %): manuelle Anpassung mit Bestätigung (V6, §8.2, BUG-6). `manual_budget_update`/`manual_budget_update_by_person` prüfen Budget (B1) und Kapazität: `> avail` warnt nur (weich, §9.2), drohende €-Budgetüberschreitung wirft ohne `confirm` `BudgetConfirmationRequired` (Router → HTTP 409 mit `warnings`-Body, kein Speichern); mit `confirm=true` wird gespeichert und `is_manual_override=True` gesetzt (jede manuelle Änderung setzt das Flag, damit Resync sie erhält). PUT `.../budgets/{id}` und `.../persons/{id}` haben Query-Param `confirm` und liefern `BudgetUpdateOut` (Budgetzeile + `warnings`). Tests: `test_services_milestones_manual.py` + Router-Tests.
>
> **WP6** ist implementiert (Suite grün: 440 Tests, Coverage 91.60 %): zielorientiertes Rebalancing + Empfehlungen + WP1-Rebalancing-Teil (V7, V8, BUG-8/9/10). `suggest_rebalancing` verteilt jetzt das Restbudget `R = Gesamt − abgerechnet(gesperrt) − gebucht(offen) − Overrides` über die offenen Meilensteine per gemeinsamer `distribute_budget`/`_person_available_hours` (Netto-Kapazität statt roh → BUG-8), reizt R maximal aus ohne Überschreitung (BUG-9), berücksichtigt Drift/gebuchte Stunden (BUG-10) und lässt Override-Zeilen fix; `MilestoneSuggestion.suggested_total_hours` neu für die „Rebalanciert"-Spalte. `apply_rebalancing` rundet nicht mehr (§9.4). Neu: `compute_recommendations` (Umkehrung von `_membership_overbooking_warnings`) + Endpoint `GET /projects/{id}/milestones/recommendations` (frei h/Woche, €-Headroom, empfohlene Zusatzstunden). Tests: erweiterte `test_services_rebalancing.py` + Router-Tests.
>
> **WP7** ist implementiert (Frontend; `tsc -b` fehlerfrei, Vitest 16 grün, Backend weiter 440 grün): `api.ts` um `priority`, `is_manual_override`, `warnings`, `suggested_total_hours`, `resyncMilestones`, `recommendations`, `updatePersonBudget(confirm)` und eine `ApiError`-Klasse (Status + Body) erweitert. `ProjectDetailPage.tsx`: „Resync"-Button + „veraltet"-Badge (Mitglied ohne Budgetzeile im überlappenden offenen Monat), Auslastungs-Empfehlungen-Hinweisbox (B4), 0-h-Warnkennzeichnung an der Monatszeile (aus `MilestoneDetail.warnings`), inline „Rebalanciert: X h (Δ)" in der Aktuell-Spalte + Vorschlagssumme im Rebalancing-Tab, „M"-Badge für manuelle Overrides, Prioritäten-Spalte + -Feld in Mitglieder-Tabelle/-Formularen, Budget-Override-Bestätigungsdialog (409 → Warnungen + „Trotzdem speichern" mit `confirm=true`). Kleine Backend-Ergänzung: `MilestonePersonDetailOut.is_manual_override`.
>
> **WP8** ist implementiert (Frontend; `tsc -b` fehlerfrei, Vitest 21 grün): dreistufiges Hilfe-Konzept (V14, §6.9). Neue wiederverwendbare Komponenten `PageIntro.tsx` (Stufe 1 — Kurzbeschreibung + „Hilfe"-Trigger), `HelpDrawer.tsx` (Stufe 3 — seitliches Panel, Escape/Backdrop-Close, `aria-modal`), `HelpPopover.tsx` (Stufe 2 — „?"-Popover mit Kurzformel). Doku-Inhalte als pflegbare TSX-Bausteine in `content/milestoneHelp.tsx` inkl. durchgerechnetem Meilenstein-Beispiel (Budget → Verfügbarkeit → Skalierung → Priorität). Eingebunden in den Meilenstein-Tab (PageIntro + Drawer) mit HelpPopover an der „Plan (Std.)"-Spalte. Komponententests in `HelpComponents.test.tsx`. Kein neues Markdown-Dependency (bewusst als TSX gehalten).
>
> **Gesamtstand:** WP1–WP8 abgeschlossen. Backend 440 Tests grün (Coverage 91.60 %, `--cov-fail-under=90`), Frontend `tsc -b` fehlerfrei + Vitest 21 grün.
